...
void guess_bit_string(const Vector3i& dims, OUT StringRef& bit_string)
{
  ...
  HANA_ASSERT(size > 0);
  HANA_ASSERT(bit_string.size > size);
  bit_string[size] = 0; // the final NULL character
...
void get_block_grid(
  StringRef bit_string, uint64_t block_number, int bits_per_block,
  OUT Vector3i& from, OUT Vector3i& to, OUT Vector3i& stride)
{
  ...
  int last_sample_level =
    (first_sample_hz == 0) ? (bits_per_block + 1) : (log_int(2, first_sample_hz) + 1);
...
bool intersect_grid(
  const Volume& vol, const Vector3i& from, const Vector3i& to, const Vector3i& stride,
